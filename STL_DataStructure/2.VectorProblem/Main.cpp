...
int main()
{
	...
	{
		std::cout << "방장은 " << roomMaster.userName << " 입니다.\n";
	}
	...
	UserInfo userInfo;
	// GameDev의 레벨 확인.
	std::cout << "[TaeJun의 정보] " << gameRoom.GetUserOfName("TaseJun", userInfo);

	// 마지막에 방에 들어온 유저 정보.
	std::cout << "[가장 마지막에 들어온 유저 정보] " << gameRoom.GetUserOfLastOrder();

	// 모든 유저 삭제.
	gameRoom.Clear();

	// 모든 유저 삭제 후 비었는지 확인.
	std::cout << (gameRoom.IsEmpty() == true ? "방이 비었습니다.\n" : "방이 비어있지 않습니다\n");
